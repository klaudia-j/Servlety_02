...
public class Post_Servlet_02 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		Set<String> setOfDirtyWords = new HashSet<>();
		for(String wulgaryzm : wulgaryzmy) {
			setOfDirtyWords.add(wulgaryzm);
		}
		...
		if(dirty == null) {
			...
			while (tok.hasMoreTokens()){
				...
				if(setOfDirtyWords.contains(token)) {
					token = "***";
					break;
				}
				result.append(token);
